...
export default class AddressDropdownPopupMenu extends Component<Props> {
	...
	renderAddressItems() {
		if (!this.props.addressList || !Array.isArray(this.props.addressList) || this.props.addressList.length <= 0) {
			return (
				<div
		...
		return this.props.addressList.map((addressItem, index) =>
			(
				...
				>
					...
					<div className={styles.itemRowBalance}>{addressItem.balance.toString()} RES</div>
				</div >
			))
	}
