...
import {
  ...
  DaemonInfo,
  SystemInfoActions,
} from "../reducers/system-info/system-info.reducer";
import {
  Balances,
  OverviewActions,
  Transaction,
} from "../reducers/overview/overview.reducer";
...
export class RpcService {
  ...
  requestDaemonInfo() {
    ...
    client
      ...
      .catch((err) => {
        ...
        getStore().dispatch(
          SystemInfoActions.getDaemonInfoFailure(
            `${errorPrefix}: ${err}`,
            err.code
          )
        );
    ...
    from(client.command(commandList))
      ...
      .subscribe(
        ...
        (error) => {
          ...
          const errorPrefix = t(`Unable to get Resistance local node info`);
          getStore().dispatch(
            OverviewActions.getWalletInfoFailure(`${errorPrefix}: ${error}`)
          );
        }
      );
  }
  ...
  requestTransactionsDataFromWallet() {
    ...
    const combineQueryPromise = Promise.all(queryPromiseArr).then((result) => {
      const combinedTransactionsList = [...result[0], ...result[1]];
      const sortedByDateTransactions = combinedTransactionsList.sort(
        (trans1, trans2) =>
    ...
    this::applyAddressBookNamesToTransactions(combineQueryPromise).subscribe(
      (result) => {
        log.debug(`Wallet transactions: ${result}`);
        getStore().dispatch(
          OverviewActions.gotTransactionDataFromWallet(result.transactions)
        );
      },
      (error) => {
        ...
        getStore().dispatch(
          OverviewActions.getTransactionDataFromWalletFailure(
            `${errorPrefix}: ${error}`
          )
        );
      }
  ...
  requestBlockchainInfo() {
    ...
    client
      ...
      .then((result) => {
        ...
        getStore().dispatch(
          SystemInfoActions.gotBlockchainInfo(blockchainInfo)
        );
        return Promise.resolve();
      })
      .catch((err) => {
        ...
        getStore().dispatch(
          SystemInfoActions.getBlockchainInfoFailure(
            `${errorPrefix}: ${err}`,
            err.code
          )
        );
  ...
  sendCurrency(fromAddress: string, toAddress: string, amountToSend: Decimal) {
    ...
    const commandParameters = [
      ...
      [
        {
          address: toAddress,
          amount: amountToSend.sub(DECIMAL.transactionFee),
        },
      ],
    ];
    ...
    if (
      fromAddress.toLowerCase().startsWith("r") &&
      toAddress.toLowerCase().startsWith("r")
    ) {
      commandParameters.push(0);
    }

    const command = client.command([
      { method: `z_sendmany`, parameters: commandParameters },
    ]);
    ...
    command
      .then(([result]) => {
        if (typeof result === "string") {
          getStore().dispatch(
            SendCurrencyActions.sendCurrencyOperationStarted(result)
          );
        } else {
          getStore().dispatch(
            SendCurrencyActions.sendCurrencyFailure(result.message)
          );
        }
        return Promise.resolve();
      })
      .catch((err) =>
        getStore().dispatch(
          SendCurrencyActions.sendCurrencyFailure(err.toString())
        )
      );
  }
  ...
  async getMyAddresses(
    ...
    disablePrivateAddresses?: boolean
  ) {
    const client = getClientInstance();
    ...
    const addresses = Array.from(
      new Set(
        Array.prototype.concat(
          ...
          z,
          groupings.reduce(
            (accumulated, value) => accumulated.concat(value.map((l) => l[0])),
            []
          )
        )
      )
    ...
    client
      ...
      .catch((err) => {
        ...
        return getStore().dispatch(
          SystemInfoActions.getOperationsFailure(
            `${errorPrefix}: ${err}`,
            err.code
          )
        );
  ...
  mergeAllMinedCoins(zAddress: string) {
    const command = getClientInstance().command(
      "z_shieldcoinbase",
      "*",
      zAddress
    );
  ...
  getTransactionDetails(transactionId: string) {
    ...
    const queryPromise = client.command([
      { method: "gettransaction", parameters: [transactionId] },
    ]);
    ...
    return from(queryPromise).pipe(
      ...
      map((result) => {
        ...
        Object.keys(result.details[0]).reduce((accumulator, key) => {
          ...
          if (key === "amount") {
            modified[`details[0].${key}`] = Decimal(
              result.details[0][`${key}`]
            );
          } else {
      ...
      catchError((error) => {
        log.debug(
          `An error occurred while fetching transcation details: ${error}`
        );
  ...
  importPrivateKeys(filePath) {
    ...
    const observable = from(
      promisify(fs.copyFile)(filePath, importFilePath).then(() =>
        client
          .command("z_importwallet", importFilePath)
...
async function getPublicTransactionsPromise(client: Client) {
  const command = [{ method: "listtransactions", parameters: ["", 200] }];
  ...
  return client
    ...
    .then((result) => {
      if (Array.isArray(result)) {
        return result.map((originalTransaction) => ({
...
async function getPrivateTransactionsPromise(client: Client) {
  ...
  const getWalletZReceivedTransactionsCmd = (zAddress) => [
    { method: "z_listreceivedbyaddress", parameters: [zAddress, 0] },
  ];
  const getWalletTransactionCmd = (transactionId) => [
    { method: "gettransaction", parameters: [transactionId] },
  ];

  // First, we get all the private addresses, and then for each one, we get all their transactions
  const privateAddresses = await client
    .command(getWalletZAddressesCmd())
    .then((tempResult) => tempResult[0]);
  ...
  if (Array.isArray(privateAddresses) && privateAddresses.length > 0) {
    ...
    for (let index = 0; index < privateAddresses.length; index += 1) {
      ...
      const addressTransactions = await client
        .command(getWalletZReceivedTransactionsCmd(tempAddress))
        .then((tempResult) => tempResult[0]);

      if (
        Array.isArray(addressTransactions) &&
        addressTransactions.length > 0
      ) {
        const addressTransactionsWithPrivateAddress = addressTransactions.map(
          (tran) => Object.assign({}, tran, { address: tempAddress })
        );
        queryResultWithAddressArr = [
          ...queryResultWithAddressArr,
          ...addressTransactionsWithPrivateAddress,
        ];
      }
    }
    ...
    for (let index = 0; index < tempTransactionsList.length; index += 1) {
      ...
      const transactionDetails = await client
        .command(getWalletTransactionCmd(tempTransaction.transactionId))
        .then((tempResult) => tempResult[0]);
...
function applyAddressBookNamesToTransactions(transactionsPromise) {
  const observable = from(transactionsPromise).pipe(
    switchMap((result) => {
      ...
      return addressBookService.loadAddressBook().pipe(
        map((addressBookRecords: AddressBookRecord[]) => {
          ...
          modified.transactions = result.transactions.map(
            (transaction: Transaction) => {
              const matchedRecord = addressBookRecords.find(
                (record) => record.address === transaction.destinationAddress
              );
              return matchedRecord
                ? { ...transaction, destinationAddress: matchedRecord.name }
                : transaction;
            }
          );
...
async function fetchAddressBalances(addresses, ledgerAddress) {
  ...
  const commands = addresses.reduce(
    (accumulated, address) =>
      accumulated.concat([
        {
          method: "z_getbalance",
  ...
  const result = addresses.map((address, index) => {
    ...
    if (
      confirmedBalance instanceof Error ||
      unconfirmedBalance instanceof Error
    ) {
      return {
        balance: null,
        confirmed: false,
...
function performMergeCoinsCommand(commandPromise: Promise<any>) {
  commandPromise
    .then((result) =>
      getStore().dispatch(
        OwnAddressesActions.mergeCoinsOperationStarted(result.opid)
      )
    )
    .catch((err) =>
...
function exportFileWithMethod(method, filePath) {
  ...
  return from(
    client.command(method, exportFileName).then((result) => {
