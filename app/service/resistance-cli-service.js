...
import {
  ...
  DaemonInfo,
  SystemInfoActions
} from '../state/reducers/system-info/system-info.reducer';
import {
  Balances,
  Transaction,
  OverviewActions
} from '../state/reducers/overview/overview.reducer';
...
export class ResistanceCliService {
      ...
      from(getPublicTransactionsPromise)
        ...
        .subscribe(
          result => {
            ...
            if (
              result.transactions &&
              (result.optionalError === null ||
                result.optionalError === undefined)
            ) {
              this.dispatchAction(
                OverviewActions.gotTransactionDataFromWallet(
                  result.transactions
                )
              );
            }
  ...
  startPollingBlockChainInfo() {
    ...
    const getAsyncBlockchainInfo = () => {
      ...
      const getBlockChainInfoPromise = cli
        .getConnectionCount()
        .then(result => {
        ...
        .then(result => {
          ...
          finalResult.blockchainSynchronizedPercentage = getBlockchainSynchronizedPercentage(
            finalResult.lastBlockDate
          );
  ...
  getAddressBalance(cli: Client, addressRow: AddressRow): Promise<any> {
    return cli
      ...
      .then(result => {
        ...
        const fixedBalanceStr =
          typeof tempBalance === 'string'
            ? parseFloat(tempBalance).toFixed(2)
    ...
    const createNewAddressPromise = cli
      ...
      .then(newAddress => {
        this.logger.debug(
          this,
          `createNewAddress`,
          `create ${isPrivate ? 'private ' : 'transparent '} address: `,
          ConsoleTheme.error,
          newAddress
        );
        return newAddress;
  ...
  sendCash(fromAddress: string, toAddress: string, amountToSend: number) {
     ...
     *
     */
    const getSendCashPromise = (
      fAddress: string,
      tAddress: string,
      amountNeedToSend: number
    ): Promise<any> => {
      ...
      return cli.command([
        { method: `z_sendmany`, parameters: sendCashParams }
      ]);
    };
      ...
      promise
        .then(result => {
          ...
          if (tempStatus && tempStatus.status === 'success') {
            ...
            const failMessage =
              tempStatus.error && tempStatus.error.message
                ? tempStatus.error.message
                : `Unknow error happen.`;
            this.dispatchAction(
              SendCashActions.sendCashFail(failMessage, true)
            );
            ...
            this.dispatchAction(
              SendCashActions.updateSendOperationStatus(inProgressOperation)
            );
          }
        ...
        .catch(error => {
          ...
          // Make sure pass "true" to "clearCurrentOperation" !!!
          this.dispatchAction(
            SendCashActions.sendCashFail(error.message, true)
          );
          this.stopPollingOperationStatus();
        });
    };

    sendCashPollingIntervalId = setInterval(
      () => getAsyncOperationStatus(operationId),
      sendCashPollingIntervalSetting
    );
  }
      ...
      .then(result => {
        ...
        if (isPrivate) {
          ...
          if (Array.isArray(privateAddressesResult)) {
            for (
              let index = 0;
              index < privateAddressesResult.length;
              index++
            ) {
              addressResultSet.add(privateAddressesResult[index]);
            }
        ...
        this.logger.debug(
          this,
          `getWalletPublicAddressAndBalance`,
          `combinedAddresses: `,
          ConsoleTheme.testing,
          combinedAddresses
        );
